...
{	
	...
	public ResponseEntity<ChatCommon> sendMessage(@Payload ChatCommon message) 
	{
		log.debug("chatCommon Chatroom_num : {}", message.getChatroom_num());
		
	    // WebSocketSession 대신 SimpMessageSendingOperations를 사용하여 메시지를 전송합니다.
		template.convertAndSend("/sub/chat/openChatroom/" + message.getChatroom_num(), "asdf");
		
		 return new ResponseEntity<ChatCommon>(HttpStatus.UNAUTHORIZED);
	}
